...
def resampling(strokes_info):
    ...
    if len(strokes_info) > 30:
        # print('error',len(strokes_info))
        strokes_info.sort(key=lambda x: len(x),reverse=True)
        strokes_info = strokes_info[:30]
        # print(len(strokes_info))
    ...
    for mark in strokes_info:
        ...
        # print(len(x),points,value_adjust)
        if len(x) < 4:
            x_resampled = []
            y_resampled = []
